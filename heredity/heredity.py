...
# Define probabilities for genes and traits
PROBS = {
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    """
    probability = 1
    # Iterate over the people dictionary
    for person, data in people.items():

        mother = data["mother"]
        father = data["father"]

        # Determine the gene count for the person
        ...
        if father is None and mother is None:
            ...
            if mother and father:
                ...
                for parent in [mother, father]:
                    # If parent has two genes, child will get the gene unless it mutates
                    if parent in two_genes: 
                        parent_prob[parent] = 1 - PROBS["mutation"]
                    elif parent in one_gene:
                        parent_prob[parent] = 0.5
                    else:
                        parent_prob[parent] = PROBS["mutation"]
                ...
                if gene_count == 2:
                    ...
                    gene_prob = (
                         parent_prob[mother] * (1 - parent_prob[father]) + (1 - parent_prob[mother]) * parent_prob[father]
                         )
                else:
                    gene_prob = (1 - parent_prob[mother]) * (1 - parent_prob[father])

        # Determine if they have the trait based on gene count and whether they actually have it
...
def normalize(probabilities):
    ...
    for person in probabilities:
        ...
        for geneNum in probabilities[person]["gene"]:
            probabilities[person]["gene"][geneNum] /= total_gene_prob
        ...
        for traitVal in probabilities[person]["trait"]:
            probabilities[person]["trait"][traitVal] /= total_trait_prob
